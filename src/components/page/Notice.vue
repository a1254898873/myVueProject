  ...
  <div class>
    <div class="crumbs">
      ...
      <el-breadcrumb separator="/">
        <el-breadcrumb-item>
          <i class="el-icon-lx-copy"></i> 我的通知
        </el-breadcrumb-item>
      </el-breadcrumb>
    </div>
    <div class="container">
          ...
          <el-table :data="unread" :show-header="false" style="width: 100%">
            <el-table-column>
              ...
              <template v-slot="scope">
                <span class="message-title">{{scope.row.notifier + " 评论了你的数据集 " + scope.row.dtitle + " : "}}</span><br>
        ...
        <el-tab-pane :label="`已读消息(${read.length})`" name="second">
          ...
          <template v-if="message === 'second'">
            <el-table :data="read" :show-header="false" style="width: 100%">
              ...
              <!-- 动态渲染data中模拟的时间 -->
              <el-table-column prop="createAt" width="150"></el-table-column>
              <el-table-column width="120">
                <template v-slot="scope">
...
export default {
  ...
  methods: {
    getUnread() {
      getRequest("/notification?status=0").then(resp => {
        var data = resp.data;
        this.unread = data.obj;
      });
    },
    getRead() {
      getRequest("/notification?status=1").then(resp => {
        var data = resp.data;
        this.read = data.obj;
      });
    },
    getRecycle() {
      getRequest("/notification?status=-1").then(resp => {
        var data = resp.data;
        this.recycle = data.obj;
      });
    },
    // 操作对应信息，标为已读，获取传入的当前信息索引
    handleRead(index) {
      getRequest("notification/status?status=1&id=" + this.unread[index].id);
    ...
    // 操作所有信息，标为已读
    handleAllRead() {
      getRequest("notifications/status?oldStatus=0&newStatus=1");
    ...
    handleDel(index) {
      getRequest("/notification/status?status=-1&id=" + this.read[index].id);
    ...
    // 操作所有信息，标为回收
    handleAllDel() {
      getRequest("/notifications/status?oldStatus=1&newStatus=-1");
    ...
    // 操作对应信息，标为已读，获取传入的当前信息索引
    handleRestore(index) {
      getRequest("/notification/status?status=1&id=" + this.recycle[index].id);
    ...
    // 操作所有信息，彻底删除
    handleAllDelete() {
      getRequest("/notifications/delete");
      // 将回收信息对象数组中所有信息全部删除
